            ...
            <div class="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="max-w-3xl space-y-4">
                    ...
                    <p class="text-slate-300 text-sm sm:text-base font-normal leading-relaxed max-w-2xl">
                        ...
                        <template v-else-if="searchQuery.trim()">
                            Showing articles matching "{{ searchQuery.trim() }}".
                        </template>
                        <template v-else>
                            In-depth guides, equipment breakdowns, scoring rules, and visual tutorials to elevate your shooting.
                        </template>
                    </p>
                </div>
            </div>
                        ...
                        <div class="pt-3 border-t border-slate-100 flex items-center justify-between gap-4">
                            <div class="flex items-center gap-2.5 min-w-0">
                                <img src="/profile-author.png" :alt="(featuredArticle.author?.name || 'Archeris Editorial Team') + ' Avatar'"
                                    class="w-8 h-8 rounded-full bg-slate-100 shrink-0 border border-slate-200 object-cover" />
                                <div class="truncate">
                                    <div class="text-navy font-bold text-xs sm:text-sm truncate">Archeris Editorial</div>
                                    <div class="text-slate-400 text-[11px] sm:text-xs font-medium truncate">Editorial Team</div>
                                </div>
                            </div>

                            <div class="shrink-0 px-4 py-2 bg-navy text-white text-xs sm:text-sm font-bold rounded-xl group-hover:bg-slate-800 transition-colors flex items-center gap-1.5 shadow-xs">
                                <span>Read Article</span>
                                <Icon icon="ph:arrow-right" class="text-xs sm:text-sm" />
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 space-y-8">
                ...
                <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1 md:pb-0">
                        <button v-for="cat in categories" :key="cat"
                            ...
                            :class="[
                                'px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap flex items-center gap-2 shrink-0 cursor-pointer',
                                activeCategory === cat
                    ...
                    <div class="relative min-w-[240px] sm:min-w-[280px]">
                        ...
                        <input v-model="searchQuery" type="text"
                            ...
                            class="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm text-navy placeholder:text-slate-400 focus:bg-white focus:border-navy focus:outline-hidden transition-colors" />
                        <button v-if="searchQuery" @click="searchQuery = ''"
                            class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-navy text-sm cursor-pointer">
            ...
            <div>
                ...
                <div v-if="filteredArticles.length === 0"
                    ...
                    <h3 class="text-base sm:text-lg font-bold text-navy">No Articles Found</h3>
                    ...
                    <button @click="resetFilters"
                        class="px-4 py-2 bg-navy text-white text-xs sm:text-sm font-bold rounded-xl hover:bg-slate-800 transition-colors cursor-pointer">
                        Reset Filters
                    </button>
                </div>
                ...
                <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                    <NuxtLink v-for="article in paginatedArticles" :key="article.slug"
                        ...
                        <div class="p-5 sm:p-6 flex-1 flex flex-col justify-between space-y-4">
                            <div>
                                <h3 class="text-base sm:text-lg font-bold text-navy leading-snug line-clamp-2 font-display mb-2">
                                    {{ article.title }}
                                </h3>
                            ...
                            <div class="pt-3 border-t border-slate-100 flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    <img src="/profile-author.png" :alt="(article.author?.name || 'Archeris Editorial') + ' Avatar'"
                                        class="w-6 h-6 rounded-full bg-slate-100 shrink-0 border border-slate-200 object-cover" />
                                    <span class="text-xs sm:text-sm text-slate-600 font-medium truncate">{{ article.author?.name || 'Archeris Editorial' }}</span>
                                </div>

                                <div class="shrink-0 flex items-center gap-1 text-xs sm:text-sm font-bold text-navy">
                                    <span>Read Article</span>
                ...
                <div v-if="totalPages > 1" class="pt-10 flex items-center justify-center gap-2">
                    ...
                    <button @click="changePage(currentPage - 1)" :disabled="currentPage === 1"
                        :class="[
                            'px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold flex items-center gap-1 transition-all cursor-pointer',
                    ...
                    <div class="flex items-center gap-1.5">
                        <button v-for="page in totalPages" :key="page"
                            ...
                            :class="[
                                'w-9 h-9 rounded-xl text-xs sm:text-sm font-bold transition-all flex items-center justify-center cursor-pointer',
                                currentPage === page
...
useHead({
    title: 'Archery Insights & Guides - Archeris',
    link: [
...
useSeoMeta({
    title: 'Archery Insights & Guides - Archeris',
    description: 'Explore comprehensive guides on archery shooting basics, equipment setup, scoring rules, and tournament strategies with Archeris.',
    ogTitle: 'Archery Insights & Guides - Archeris',
    ogDescription: 'Explore comprehensive guides on archery shooting basics, equipment setup, scoring rules, and tournament strategies with Archeris.',
    ogType: 'website',
