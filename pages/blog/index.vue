            ...
            <div class="container mx-auto px-4 max-w-7xl">
                <NuxtLink :to="`/blog/${featuredArticle.slug}`" class="group block relative rounded-[40px] overflow-hidden bg-navy">
                    ...
                    <div class="absolute inset-0 p-8 md:p-16 flex flex-col justify-end">
                        ...
                        <h1 class="text-white text-3xl md:text-6xl font-black leading-[1.1] max-w-4xl tracking-tight group-hover:text-primary transition-colors">
                        ...
                        <div class="flex items-center gap-4 mt-8 pt-8 border-t border-white/10">
                            <img :src="featuredArticle.author.avatar" class="w-10 h-10 rounded-full border-2 border-primary/30" />
                            <div>
                                <div class="text-white font-black text-sm">{{ featuredArticle.author.name }}</div>
                                <div class="text-white/40 text-[10px] font-bold tracking-widest uppercase">Spesialis Panahan</div>
                            </div>
                        </div>
                    </div>
                </NuxtLink>
        ...
        <section class="container mx-auto px-4 max-w-7xl pb-32">
            ...
            <div class="flex items-center justify-between mb-12 border-b border-navy/5 pb-8">
                ...
                <div class="hidden md:flex items-center gap-2 text-navy/20">
                    ...
                    <input v-model="searchQuery" type="text" placeholder="Cari artikel..." class="bg-transparent border-none outline-none text-[10px] font-black uppercase tracking-widest text-navy placeholder:text-navy/10 w-48" />
                </div>
            </div>
                ...
                <NuxtLink v-for="article in filteredArticles" :key="article.slug" :to="`/blog/${article.slug}`"
                    ...
                    <div class="relative aspect-video overflow-hidden bg-slate-100">
                        ...
                        <div class="absolute top-4 left-4">
                            <span class="bg-white/90 backdrop-blur-md text-navy text-[8px] font-black px-3 py-1 rounded-lg tracking-widest uppercase border border-navy/5">
                                {{ article.category }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="p-8 flex flex-col flex-1">
                        <div class="text-navy/20 text-[9px] font-black tracking-widest uppercase mb-4">{{ article.date }}</div>
                        <h3 class="text-navy text-xl font-black leading-tight group-hover:text-primary transition-colors line-clamp-2 mb-4">
                            {{ article.title }}
                        </h3>
                        ...
                        <div class="mt-auto pt-6 border-t border-navy/5 flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <img :src="article.author.avatar" class="w-6 h-6 rounded-full grayscale group-hover:grayscale-0 transition-all" />
                                <span class="text-[10px] font-black text-navy/40 uppercase tracking-widest">{{ article.author.name }}</span>
                            </div>
                            <Icon icon="ph:arrow-up-right-bold" class="text-navy/10 group-hover:text-primary group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" />
                        </div>
                    </div>
                </NuxtLink>
            </div>
                    ...
                    <div>
                        <form @submit.prevent="handleSubscribe" class="flex flex-col sm:flex-row gap-4">
                            <input v-model="subscribeEmail" type="email" placeholder="Alamat email Anda..." 
                                class="flex-1 bg-white border-none rounded-2xl px-6 py-4 text-navy font-bold focus:ring-4 focus:ring-navy/10 outline-none" required />
                            <button class="bg-navy text-white px-10 py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-navy-light transition-all active:scale-95">
                                Gabung Sekarang
                            </button>
                        </form>
                    </div>
