...
const isCategoryExpanded = (catName) => expandedCategories.value.has(catName)

const categoryBreakdown = computed(() => {
  if (!payment.value?.participants || payment.value.participants.length === 0) return []
  const map = new Map()
  for (const p of payment.value.participants) {
    const catName = p.category_name || t('org_event_payments.general_category') || 'Kategori Umum'
    if (!map.has(catName)) {
      map.set(catName, {
    ...
    group.archers.push(p)
    group.total_fee += Number(p.payment_amount || p.cost || 0)
    if (p.last_reregistration_at) {
      group.checked_in_count += 1
    }
  }
  return Array.from(map.values())
})

// Auto expand all categories upon data load
watch(categoryBreakdown, (cats) => {
  if (cats && cats.length > 0) {
    cats.forEach(c => expandedCategories.value.add(c.category_name))
  }
}, { immediate: true })
...
const getStatusLabel = (status) => {
  ...
  if (s === 'failed') return t('payment_status.badge_failed', 'Gagal')
  ...
  <div class="flex flex-col gap-6 md:gap-8 pb-16 font-body">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div class="flex items-center gap-2.5 flex-wrap">
            ...
            <a
              ...
              target="_blank"
              class="px-4 py-2.5 bg-primary hover:bg-primary/90 text-btn-text rounded-xl text-xs sm:text-sm font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
            >
              <Icon icon="ph:receipt-bold" class="size-4" />
              <span>{{ t('org_event_payments.view_invoice_pdf') || 'Cetak Invoice' }}</span>
            </a>
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
      ...
      <div class="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm divide-y divide-slate-100 dark:divide-slate-700 overflow-hidden">
        ...
        <div class="p-5 sm:p-6 space-y-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs sm:text-sm">
            <div class="p-4 bg-slate-50 dark:bg-slate-700/40 rounded-xl space-y-1 border border-slate-100 dark:border-slate-700">
              <div class="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">{{ t('org_event_payments.account_registrant_label') }}</div>
            ...
            <div class="p-4 bg-slate-50 dark:bg-slate-700/40 rounded-xl space-y-1 border border-slate-100 dark:border-slate-700">
              <div class="text-xs text-slate-500 dark:text-slate-400 font-medium block mb-1">{{ t('org_event_payments.account_sender_label') }}</div>
              <div class="font-bold text-navy dark:text-white text-sm sm:text-base">
        ...
        <!-- Section 2: Registration Details (Rincian Pendaftaran) Grouped by Category -->
        <div class="space-y-0">
          <div class="p-5 sm:p-6 flex items-center justify-between border-b border-slate-100 dark:border-slate-700 flex-wrap gap-3">
            <div class="flex items-center gap-3">
              <div class="size-10 rounded-xl bg-primary text-btn-text flex items-center justify-center shrink-0 shadow-2xs font-bold">
          ...
          <!-- Empty State -->
          <div v-if="!categoryBreakdown || categoryBreakdown.length === 0" class="p-8 text-center text-slate-400 italic text-xs sm:text-sm">
          ...
          <div v-else class="divide-y divide-slate-100 dark:divide-slate-700">
            ...
            >
              <!-- Category Header / Expand Toggle Row -->
              <div
                @click="toggleCategory(cat.category_name)"
                class="p-4 sm:p-5 flex items-center justify-between gap-4 cursor-pointer hover:bg-slate-50/80 dark:hover:bg-slate-750 select-none transition-colors"
              >
                <div class="flex items-center gap-3.5 min-w-0">
                  <div
                    class="size-7 rounded-lg bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 flex items-center justify-center shrink-0 text-slate-500 dark:text-slate-300 transition-transform duration-200"
                    :class="{ 'rotate-180': isCategoryExpanded(cat.category_name) }"
                  >
                    <Icon icon="ph:caret-down-bold" class="size-3.5" />
                  </div>

                  <div class="min-w-0 space-y-0.5">
                    <div class="flex items-center gap-2 flex-wrap">
                      <span class="font-bold text-sm sm:text-base text-navy dark:text-white truncate">
                        {{ cat.category_name }}
                      </span>
                      <span v-if="cat.event_type_name" class="text-xs px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-medium">
                        {{ cat.event_type_name }}
                      </span>
                    </div>
                    <div class="flex items-center gap-2 text-xs text-slate-400 flex-wrap">
                      <span>{{ cat.archers.length }} {{ t('org_event_payments.th_archer', 'Atlet') }}</span>
                      <span>&bull;</span>
                      <span :class="cat.checked_in_count === cat.archers.length && cat.archers.length > 0 ? 'text-emerald-600 dark:text-emerald-400 font-semibold' : 'text-slate-500 dark:text-slate-400'">
                        {{ cat.checked_in_count }}/{{ cat.archers.length }} {{ t('org_event_payments.checked_in', 'Check-in') }}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div class="text-right shrink-0">
                  <div class="text-xs text-slate-400 font-medium hidden sm:block">{{ t('org_event_payments.th_subtotal', 'Subtotal') }}</div>
                  <div class="font-bold text-navy dark:text-white text-sm sm:text-base">
                    {{ formatCurrency(cat.total_fee) }}
                  </div>
                </div>
              </div>

              <!-- Collapsible Archers Table -->
              <div
                v-if="isCategoryExpanded(cat.category_name)"
                class="bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700 overflow-x-auto"
              >
                <table class="w-full text-left text-xs sm:text-sm">
                  <thead class="bg-slate-100/70 dark:bg-slate-700/40 text-slate-500 dark:text-slate-400 text-xs font-semibold">
                    <tr>
                      <th class="py-2.5 px-4 w-12 text-center">#</th>
                      <th class="py-2.5 px-4">{{ t('org_event_payments.th_athlete_name') }}</th>
                      <th class="py-2.5 px-4 text-center">{{ t('org_event_payments.th_checkin_status') }}</th>
                      <th class="py-2.5 px-4 text-right">{{ t('org_event_payments.th_fee') }}</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                    <tr
                      v-for="(p, pIdx) in cat.archers"
                      :key="p.uuid || p.id || pIdx"
                      class="hover:bg-slate-100/40 dark:hover:bg-slate-700/30 transition-colors"
                    >
                      <td class="py-2.5 px-4 text-center text-slate-400 font-medium text-xs">{{ pIdx + 1 }}</td>
                      ...
                      <td class="py-2.5 px-4 text-center">
                        <span
                          ...
                          :class="p.last_reregistration_at ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300' : 'bg-slate-200/70 text-slate-600 dark:bg-slate-700 dark:text-slate-300'"
                        >
                      ...
                      <td class="py-2.5 px-4 text-right font-bold text-navy dark:text-white">
      ...
      <!-- Right 1 Col: Payment Activity Timeline -->
      <div class="space-y-6">
