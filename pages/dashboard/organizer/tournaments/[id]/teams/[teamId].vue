      ...
      <div class="relative p-6 sm:p-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div class="flex items-center gap-4">
            ...
            <div>
              <h1 class="text-2xl sm:text-3xl font-black tracking-tight">{{ team?.name || t('org_team_detail.team_detail') }}</h1>
              <div class="text-slate-300 text-sm mt-1">{{ team?.category_name || t('org_team_detail.team_category') }} • {{ team?.club_name || t('event_teams.club') }}</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div v-else class="grid gap-3">
            <div v-for="(m, idx) in team.members" :key="m.uuid || idx" class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-700/50">
              ...
              <div class="font-mono font-black text-primary text-sm">
                {{ t('org_team_detail.score') }}: {{ m.score || 0 }}
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="space-y-6">
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 p-6 shadow-sm space-y-4">
          <div class="flex items-center gap-3">
            ...
            <h4 class="font-black text-navy dark:text-white text-base">{{ t('org_team_detail.team_summary') }}</h4>
          </div>
          <div class="space-y-3 text-xs">
            ...
            <div class="flex justify-between py-2">
              <span class="text-slate-500 font-bold">{{ t('org_team_detail.team_qualification_rank') }}</span>
...
useHead({
  title: () => (team.value?.name ? `Team ${team.value.name}` : t('org_team_detail.team_detail')) + ' - Archeris Dashboard'
})
...
async function fetchTeam() {
  ...
  try {
    const res = await get(`/tournaments/${eventId.value}/teams/${teamId.value}`)
    const teamData = res?.team || res?.data || res
    const membersData = res?.members || teamData?.members || []
    team.value = {
      ...teamData,
      name: teamData?.name || teamData?.team_name || '',
      rank: teamData?.rank ?? teamData?.team_rank,
      x_count: teamData?.x_count ?? teamData?.total_x_count ?? 0,
      members: membersData
    }
