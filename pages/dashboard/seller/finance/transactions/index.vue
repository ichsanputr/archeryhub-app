  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 group hover:border-primary transition-all">
            <div class="bg-green-50 p-2.5 rounded-xl text-green-600 group-hover:bg-green-500 group-hover:text-white transition-colors">
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 group hover:border-primary transition-all">
            <div class="bg-red-50 p-2.5 rounded-xl text-red-600 group-hover:bg-red-500 group-hover:text-white transition-colors">
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 group hover:border-primary transition-all">
            <div class="bg-navy/5 p-2.5 rounded-xl text-navy group-hover:bg-navy group-hover:text-white transition-colors">
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden min-h-[500px]">
        <div class="p-5 border-b border-gray-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-xs font-black text-navy tracking-[0.2em] uppercase">Log Transaksi Terkini</h2>
            </div>
            <div class="flex items-center gap-2">
                <BaseSelect v-model="filterType" :items="typeOptions" class="w-40 !mb-0" item-title="title" dense />
                <BaseButton variant="white" icon="ph:arrows-clockwise-bold" @click="fetchTransactions" class="h-10 w-10 !p-0 border-gray-100 text-gray-400" />
            </div>
        </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    <tr v-if="isLoading">
                        <td colspan="5" class="py-20 text-center">
                            <Icon icon="ph:circle-notched-bold" class="text-3xl text-primary animate-spin mx-auto mb-2" />
                            <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Memuat Transaksi...</div>
                        </td>
                    </tr>
                    <tr v-else-if="filteredTransactions.length === 0">
                        <td colspan="5" class="py-20 text-center">
                            ...
                            <div v-if="filterType !== 'all'" class="text-[10px] text-gray-300 font-medium mt-1">Coba ubah filter atau reset pencarian</div>
                        </td>
                    </tr>
                    <tr v-for="item in filteredTransactions" :key="item.id" class="hover:bg-gray-50/50 transition-colors group">
                        <td class="px-8 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="text-[11px] font-black text-navy uppercase tracking-tight">{{ item.type === 'income' ? 'Masuk' : 'Keluar' }}</div>
                            </div>
                        </td>
                        <td class="px-8 py-4">
                            <div class="text-[11px] font-bold text-navy/70">{{ formatDate(item.date) }}</div>
                            <div class="text-[9px] text-gray-400 font-bold uppercase">{{ formatTime(item.date) }}</div>
                        </td>
                        <td class="px-8 py-4">
                            <div class="text-[11px] font-black leading-none mb-1 text-navy">#{{ item.ref }}</div>
                            <div class="text-[10px] text-gray-400 font-medium max-w-[200px] truncate">{{ item.notes || 'Penghasilan Pesanan' }}</div>
                        </td>
                        <td class="px-8 py-4 text-right">
                           <div :class="item.type === 'income' ? 'text-green-600' : 'text-navy'" class="text-xs font-black">
                                {{ item.type === 'income' ? '+' : '-' }} Rp {{ item.amount.toLocaleString('id-ID') }}
                           </div>
                        </td>
                        <td class="px-8 py-4 text-center">
                            <span :class="getStatusClass(item.status)" class="px-2 py-1 rounded-md text-[8px] font-black tracking-widest border uppercase">
                                {{ item.status }}
                            </span>
                        </td>
                    </tr>
