<template>
  <div class="space-y-6 pb-20">
    ...
    <div class="relative overflow-hidden rounded-[2rem] border border-primary/20 bg-navy text-white shadow-sm transition-all duration-300">
      ...
      <div class="relative p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="flex items-center gap-4">
          <NuxtLink to="/dashboard/seller/orders" class="size-10 sm:size-12 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center shrink-0 shadow-sm hover:bg-white/20 transition-all">
            <Icon icon="ph:arrow-left-bold" class="text-primary text-xl" />
          </NuxtLink>
          <div>
            <div class="flex flex-wrap items-center gap-2 mb-0.5">
                <h1 class="text-xl sm:text-2xl font-black tracking-tight leading-tight">Detail Pesanan</h1>
                <div v-if="order" :class="getStatusClass(order.status)" class="px-2.5 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border border-current shadow-sm backdrop-blur-md">
                  {{ getStatusLabel(order.status) }}
                </div>
            </div>
            <div class="text-slate-400 text-[10px] sm:text-xs font-bold tracking-wide">#{{ (order?.id || '').slice(0, 8).toUpperCase() }} • {{ formatDate(order?.created_at) }}</div>
          </div>
        </div>
        <BaseButton variant="primary" icon="ph:printer-bold" @click="window.print()"
          class="h-10 px-6 font-black uppercase tracking-widest text-[10px] shadow-sm w-full md:w-auto">
          Cetak Invoice
        </BaseButton>
      </div>
    </div>
    ...
    <!-- Main Content Grid -->
    <div v-if="isLoading" class="min-h-[400px] flex flex-col items-center justify-center gap-4 bg-white/50 backdrop-blur-xl rounded-[2.5rem] border-2 border-dashed border-gray-100 shadow-sm">
      <div class="relative size-16">
          <div class="absolute inset-0 rounded-2xl bg-primary/10 animate-ping"></div>
          <div class="relative size-full rounded-2xl bg-white shadow-sm flex items-center justify-center border border-gray-100">
    ...
    <div v-else-if="!order" class="min-h-[400px] flex flex-col items-center justify-center gap-6 bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden relative">
       ...
       <div class="text-center space-y-1">
           ...
           <p class="text-gray-400 text-xs font-medium max-w-[240px] mx-auto">Tautan mungkin sudah tidak valid atau telah dihapus.</p>
       </div>
       <NuxtLink to="/dashboard/seller/orders">
          <BaseButton variant="primary" size="sm" icon="ph:arrow-left-bold" class="h-11 px-6 !rounded-xl font-black tracking-widest uppercase text-[10px] shadow-sm">Kembali</BaseButton>
       </NuxtLink>
    </div>
    ...
    <div v-else class="grid grid-cols-1 xl:grid-cols-12 gap-6 animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div class="xl:col-span-8 space-y-6">
        ...
        <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 group hover:border-primary/20 transition-all duration-300">
            <div class="flex items-center gap-5 w-full">
                <div class="size-14 rounded-2xl flex items-center justify-center shadow-sm shrink-0" :class="getStatusBgClass(order.status)">
                    <Icon :icon="getStatusIcon(order.status)" class="text-2xl" />
                </div>
                <div>
                    <div class="text-navy font-black text-lg tracking-tight">Status: {{ getStatusLabel(order.status) }}</div>
                    <div class="text-[11px] font-bold text-gray-400 max-w-sm leading-tight">{{ getStatusMessage(order.status) }}</div>
                </div>
            </div>
            
            <div class="flex items-center gap-2 w-full md:w-auto">
                <div class="relative flex-1 md:w-44">
                    <BaseSelect v-model="statusUpdate" :items="statusOptions" class="!h-11 !rounded-xl bg-gray-50 border-gray-100 text-xs" hide-label />
                </div>
                <BaseButton variant="primary" icon="ph:arrow-clockwise-bold" @click="handleUpdateStatus" :loading="isUpdating"
        ...
        <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
          <div class="p-6 sm:p-8 border-b border-gray-50 flex items-center justify-between bg-gray-50/20">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <div class="text-[9px] text-gray-400 font-black uppercase tracking-widest">Total {{ order.items.length }} Produk</div>
              </div>
            </div>
            <div class="hidden sm:block text-right">
               <div class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-0.5">Invoice</div>
               <div class="font-mono text-[11px] font-black text-navy opacity-60">INV/{{ new Date(order.created_at).getFullYear() }}/{{ (order.id || '').toUpperCase().slice(0,6) }}</div>
            </div>
          </div>

          <div class="p-4 sm:p-8 space-y-4">
             <div v-for="item in order.items" :key="item.id" class="relative group p-4 sm:p-6 rounded-3xl border border-gray-50 hover:bg-gray-50/30 transition-all duration-300 flex flex-col sm:flex-row items-center gap-6">
                <div class="size-24 sm:size-28 rounded-2xl bg-white p-1.5 border border-gray-100 overflow-hidden shrink-0 shadow-sm">
                ...
                <div class="flex-1 text-center sm:text-left min-w-0 w-full">
                   <div class="text-navy font-black text-base sm:text-lg tracking-tight truncate mb-2">{{ item.product_name }}</div>
                   <div class="flex flex-wrap items-center justify-center sm:justify-start gap-3">
                      <div class="px-3 py-1.5 bg-white rounded-lg border border-gray-100 text-[10px] font-bold text-gray-400 flex items-center gap-2">
                ...
                <div class="text-center sm:text-right pt-3 sm:pt-0 border-t sm:border-t-0 border-gray-100 w-full sm:w-auto">
          ...
          <div class="bg-navy p-8 sm:p-10 mt-2 relative overflow-hidden">
             ...
             <div class="flex flex-col sm:flex-row justify-between items-center sm:items-end gap-6 sm:gap-10 relative z-10">
                <div class="space-y-3 w-full sm:w-64">
                    <div class="flex justify-between items-center text-[10px] text-white/30 font-bold tracking-widest uppercase">
                        <span>Produk</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6 space-y-6 group hover:border-primary/20 transition-all duration-300">
                 <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="size-10 rounded-xl bg-navy/5 flex items-center justify-center group-hover:bg-primary/10 transition-colors">
                    ...
                    <BaseButton variant="white" size="xs" icon="ph:copy-bold" class="h-8 px-3 !rounded-lg text-[8px] font-black uppercase bg-gray-50 border-gray-100">Salin</BaseButton>
                 </div>
                 ...
                 <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-2">
                       <div class="flex-1 p-3 bg-gray-50/50 rounded-xl border border-gray-100 flex items-center gap-2.5">
                          <div class="size-8 rounded-lg bg-white flex items-center justify-center text-primary border border-gray-100"><Icon icon="ph:phone-bold" class="text-sm" /></div>
                          <div class="min-w-0">
                       ...
                       <div class="flex-1 p-3 bg-gray-50/50 rounded-xl border border-gray-100 flex items-center gap-2.5">
                          <div class="size-8 rounded-lg bg-white flex items-center justify-center text-primary border border-gray-100"><Icon icon="ph:envelope-simple-bold" class="text-sm" /></div>
                          <div class="min-w-0">
                            <div class="text-[8px] font-black text-gray-400 uppercase tracking-widest">Email</div>
                            <div class="text-[10px] font-black text-navy truncate">{{ (order.customer_email || '').split('@')[0] }}...</div>
                          </div>
                       </div>
                    </div>
                 </div>
            ...
            <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6 space-y-6 group hover:border-primary/20 transition-all duration-300">
                 ...
                 <div class="space-y-4 relative ml-1">
                    <div class="absolute left-4 top-2 bottom-2 w-px bg-gray-100 group-hover:bg-primary/10 transition-colors"></div>
                    <div v-for="(step, idx) in logSteps" :key="idx" class="relative flex items-center gap-4 group/step">
                        <div class="size-8 rounded-full border-2 border-white shadow-sm z-10 flex items-center justify-center transition-all duration-500 shrink-0"
                        ...
                        <div class="transition-all duration-300" :class="step.active ? 'translate-x-0.5' : 'opacity-40'">
                            <div class="text-[10px] font-black uppercase tracking-wider" :class="step.active ? 'text-navy' : 'text-gray-400'">{{ step.label }}</div>
         ...
         <div class="bg-white rounded-[2.5rem] p-1 border border-gray-100 shadow-sm overflow-hidden group">
            <div class="p-6 pb-8 flex flex-col items-center text-center relative">
                ...
                <div class="relative z-10">
                    <div class="size-20 sm:size-24 rounded-[1.5rem] bg-white p-1.5 shadow-sm mb-4 transition-transform duration-500">
                        <div class="size-full rounded-2xl bg-primary/20 flex items-center justify-center text-primary text-2xl font-black border border-primary/10">
                    ...
                    <div class="px-3 py-1 rounded-full bg-gray-50 border border-gray-100 text-[8px] font-black text-gray-400 uppercase tracking-widest inline-block shadow-sm">Loyal Customer</div>
                </div>
            </div>
            ...
            <div class="px-6 pb-8 space-y-3">
                ...
                <BaseButton variant="white" block class="h-11 font-black text-[10px] !rounded-xl tracking-widest uppercase !border-gray-100 hover:!border-primary hover:!bg-primary/5 hover:!text-primary transition-all">
            ...
            <div class="relative z-10 space-y-6">
               ...
               <div class="space-y-3">
                  ...
                  <div class="flex items-center justify-between py-1.5 border-b border-white/5">
                     ...
                     <span class="text-[8px] font-black uppercase px-2 py-0.5 rounded-full border border-primary/30 text-primary bg-primary/5 shadow-sm">ESCROW</span>
                  </div>
                  <div class="flex items-center justify-between py-1.5">
                     <span class="text-[10px] font-bold opacity-40 uppercase tracking-widest">Waktu</span>
                     <span class="text-[10px] font-black text-white/80">{{ formatDate(order.created_at).split('•')[0] }}</span>
                  </div>
               </div>
...
<style scoped>
@media print {
  .pb-24, .BaseButton, .BaseSelect, .sticky { display: none !important; }
  .space-y-8 { margin: 0 !important; }
  .xl\:col-span-4 { display: none !important; }
  .xl\:col-span-8 { width: 100% !important; }
}
</style>
