                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                    ...
                    <div v-else class="space-y-6">
                        <div v-for="(group, groupName) in groupedChannels" :key="groupName">
                            <h4 class="text-[10px] font-black text-slate-400 tracking-widest mb-3 px-1">{{ groupName }}
                            </h4>
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                <button v-for="channel in group" :key="channel.code" @click="form.method = channel.code"
                                    ...
                                    <img :src="channel.icon_url" :alt="channel.name"
                                        class="h-6 w-auto mb-2 grayscale group-hover:grayscale-0 transition-all"
                                        :class="{ 'grayscale-0': form.method === channel.code }" />
                                    <span class="text-[10px] font-bold text-navy text-center">{{ channel.name }}</span>

                                    <!-- Selected Indicator -->
                                    <div v-if="form.method === channel.code"
                                        class="absolute -top-2 -right-2 bg-primary text-white p-1 rounded-full shadow-sm">
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 ring-4 ring-primary/5">
                        ...
                        <div class="space-y-4 mb-8">
                            <div v-for="item in productCart" :key="item.uuid"
                                class="flex justify-between text-xs items-start gap-4">
                                <div class="flex-1 min-w-0">
                                    <p class="font-bold text-navy truncate">{{ item.product_name }}</p>
                                    <p class="text-slate-400 mt-0.5">{{ item.quantity }}x @ Rp {{
                                        formatPrice(item.product_sale_price || item.product_price) }}</p>
                                </div>
                                <span class="font-bold text-navy shrink-0">Rp {{ formatPrice((item.product_sale_price ||
