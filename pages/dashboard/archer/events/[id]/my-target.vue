    ...
    <div
      ...
      <div class="relative p-6 sm:p-8">
        <div class="flex items-center gap-2 text-sm text-white/60 mb-4">
          <NuxtLink to="/dashboard/archer/events" class="hover:text-white transition-colors">{{ t('my_target.nav_event', 'Event Saya') }}</NuxtLink>
...
useHead({ 
  title: computed(() => (t('my_target.title', 'Target & Jadwal Penembakan')) + ' - ArcheryHub Dashboard') 
})
...
async function fetchInitialData() {
  ...
  try {
    const [eventRes, participantRes, targetRes] = await Promise.allSettled([
      get<any>(`/events/${eventId.value}`),
      get<any>(`/events/${eventId.value}/participants/me`),
      get<any>(`/events/${eventId.value}/my-target`)
    ])
