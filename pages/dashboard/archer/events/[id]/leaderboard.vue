          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              <nav class="flex text-[10px] font-bold text-white/40 tracking-widest mb-1 items-center gap-1.5">
                <NuxtLink :to="`/dashboard/archer/events/${eventId}/overview`" class="hover:text-white transition-colors">
...
async function fetchCategories() {
  try {
    const res = await $api(`/events/${eventId.value}/categories`)
...
async function fetchLeaderboard() {
  ...
  try {
    const res = await $api(`/events/${eventId.value}/results/qualification`, {
