      ...
      >
        ...
        <div class="px-5 pt-5 pb-4 border-b border-gray-200 bg-white shrink-0">
          <div class="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <div class="text-[11px] text-gray-500 font-semibold mt-0.5">Percakapan dengan penjual</div>
            </div>
          </div>
          <div class="relative">
        ...
        <div class="flex-grow overflow-y-auto no-scrollbar">
          ...
          <div v-else-if="filteredConvs.length === 0" class="flex flex-col items-center justify-center p-10 text-center h-full">
            ...
            <div class="text-[11px] text-gray-400 font-medium mt-1 mb-4">Chat penjual dari halaman produk</div>
            <BaseButton to="/products" variant="primary" size="sm" icon="ph:storefront-bold"
              class="!rounded-xl">
              Jelajahi Produk
            </BaseButton>
          </div>
            ...
            <div v-for="conv in filteredConvs" :key="conv.id" @click="openConversation(conv)"
              ...
              <div class="relative size-12 shrink-0">
                <div class="size-full rounded-2xl bg-white overflow-hidden border-2"
                  ...
                  <img v-if="conv.seller_avatar" :src="conv.seller_avatar" class="w-full h-full object-cover" />
                ...
                <div class="absolute -bottom-0.5 -right-0.5 size-3.5 rounded-full border-2 border-white"
                  :class="isOnline(conv.seller_last_seen) ? 'bg-green-400' : 'bg-gray-300'"></div>
                <!-- Unread badge -->
                <div v-if="conv.archer_unread > 0"
                  ...
                  <span class="text-[9px] font-black text-navy">{{ conv.archer_unread > 9 ? '9+' : conv.archer_unread }}</span>
                </div>
              </div>
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-0.5">
                  <span class="text-sm font-black text-navy truncate">{{ conv.seller_name || 'Penjual' }}</span>
                  <span class="text-[10px] font-semibold shrink-0 ml-2"
                    :class="conv.archer_unread > 0 ? 'text-primary' : 'text-gray-400'">
        ...
        <template v-else>
          ...
          <div class="px-5 py-3 bg-white border-b border-gray-200 flex items-center gap-3 shrink-0">
            ...
            <div class="size-10 rounded-xl overflow-hidden bg-gray-100 border-2 border-gray-200 shrink-0">
              <img v-if="activeConv.seller_avatar" :src="activeConv.seller_avatar" class="w-full h-full object-cover" />
              <div v-else class="w-full h-full flex items-center justify-center">
                <Icon icon="ph:storefront-bold" class="text-gray-400" />
              </div>
            </div>
            <!-- Info -->
            <div class="flex-1 min-w-0">
              <h3 class="text-sm font-black text-navy leading-none truncate">{{ activeConv.seller_name || 'Penjual' }}</h3>
              <div class="flex items-center gap-1 mt-0.5">
                <div class="size-1.5 rounded-full"
                  :class="isOnline(activeConv.seller_last_seen) ? 'bg-green-400' : 'bg-gray-300'"></div>
                <span class="text-[10px] font-semibold"
                  :class="isOnline(activeConv.seller_last_seen) ? 'text-green-500' : 'text-gray-400'">
                  {{ lastActiveLabel(activeConv.seller_last_seen) }}
                </span>
            ...
            <a v-if="activeConv.product_id" :href="`/products/${activeConv.product_id}`" target="_blank"
              class="size-10 rounded-xl overflow-hidden border-2 border-gray-200 shrink-0 hover:border-primary/40 transition">
              ...
              <div v-else class="w-full h-full bg-gray-50 flex items-center justify-center">
                <Icon icon="ph:package-bold" class="text-gray-400" />
              </div>
            </a>
          </div>
...
const openConversation = async (conv) => {
  ...
  if (idx >= 0) conversations.value[idx].archer_unread = 0
