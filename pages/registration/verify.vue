  ...
  <div class="min-h-screen flex flex-col -m-8">
    ...
    <header class="flex items-center justify-between border-b border-brand-border px-6 py-3 bg-background-dark">
      <div class="flex items-center gap-4">
        ...
        <h2 class="text-white text-lg font-bold">Archery Hub <span class="text-brand-gold font-normal text-sm ml-2">Command Center</span></h2>
      </div>
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <aside class="w-80 border-r border-brand-border flex flex-col bg-surface-dark">
        ...
        <div class="flex-1 overflow-y-auto p-2 space-y-1">
          ...
          >
            <div class="size-10 rounded-full bg-gray-600 shrink-0 flex items-center justify-center text-white font-bold">
              {{ item.initials }}
            </div>
            <div class="flex flex-col min-w-0">
              ...
              <div class="flex items-center gap-1 mt-1">
                <span class="size-2 rounded-full" :class="item.status === 'pending' ? 'bg-primary animate-pulse' : 'bg-green-500'"></span>
                <span class="text-[10px] font-medium" :class="item.status === 'pending' ? 'text-primary' : 'text-green-500'">
                  {{ item.statusLabel }}
                </span>
              </div>
            </div>
      ...
      <main class="flex-1 flex flex-col min-w-0">
        ...
        <div class="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div class="flex-1 flex flex-col bg-gray-900 relative overflow-hidden">
            <!-- Toolbar -->
            <div class="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 px-2 py-1.5 bg-black/60 backdrop-blur-md rounded-full z-20 opacity-0 hover:opacity-100 transition-opacity">
            ...
            <div class="flex-1 flex items-center justify-center p-8 bg-[#151515]">
              <img 
                src="https://via.placeholder.com/600x800/1a1a1a/333?text=Payment+Receipt"
                alt="Payment receipt"
          ...
          <div class="flex-1 lg:max-w-[600px] flex flex-col bg-background-dark overflow-y-auto">
            <div class="p-6 space-y-8">
              <!-- Applicant Details -->
              <section>
                <h4 class="text-xs font-bold uppercase tracking-wider text-brand-gold mb-4 border-b border-brand-border pb-2">Applicant Details</h4>
              ...
              <section>
                <h4 class="text-xs font-bold uppercase tracking-wider text-brand-gold mb-4 border-b border-brand-border pb-2">Payment Reconciliation</h4>
                <div class="grid grid-cols-2 gap-4">
                  ...
                  <div class="p-4 rounded-lg bg-surface-dark border border-primary/40">
                    ...
                    <input 
                      v-model="currentApplication.paidAmount"
                ...
                <div class="mt-4">
                  ...
                  <input 
                    v-model="currentApplication.refId"
            ...
            <div class="mt-auto p-4 border-t border-brand-border bg-[#1f1b0e] flex gap-3 sticky bottom-0">
              <button class="flex-1 h-14 rounded-lg border border-red-900/30 bg-red-900/10 text-red-600 hover:bg-red-900/20 transition-colors font-bold">
                Reject
              </button>
              <button class="flex-1 h-14 rounded-lg border border-brand-border bg-surface-dark text-white hover:bg-surface-highlight transition-colors font-bold">
                Request Info
              </button>
              <button class="flex-[2] h-14 rounded-lg bg-primary text-background-dark hover:bg-yellow-400 shadow-lg shadow-primary/20 transition-all font-bold flex items-center justify-center gap-2">
