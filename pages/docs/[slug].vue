<template>
    <div class="min-h-screen bg-gray-50 docs-page">
        <!-- Breadcrumb bar -->
        <div class="fixed top-16 left-0 right-0 z-40 bg-white/90 backdrop-blur-sm border-b border-gray-100">
        ...
        <div class="container mx-auto px-4 max-w-7xl pt-[6.5rem] pb-20 flex gap-0">
            <!-- Left Sidebar -->
            <aside
                class="hidden lg:flex flex-col w-64 xl:w-72 shrink-0 sticky top-[6.5rem] h-[calc(100vh-6.5rem)] overflow-y-auto pr-4 scrollbar-styled">
                <!-- Search -->
                ...
                <div v-for="cat in sidebarCategories" :key="cat.id" class="mb-4">
                    <div class="flex items-center gap-2 px-2 py-1.5 mb-1">
                        ...
                        <span class="text-xs font-black text-gray-400 tracking-widest uppercase">{{ cat.label }}</span>
                    </div>
                    <div class="space-y-0.5">
                        <NuxtLink v-for="doc in filteredSidebarDocs(cat.id)" :key="doc.slug" :to="`/docs/${doc.slug}`"
                            ...
                            <Icon :icon="doc.icon" class="text-base shrink-0"
                                :class="currentSlug === doc.slug ? 'text-primary' : 'text-gray-400 group-hover:text-navy'" />
            ...
            <!-- Main content -->
            <main class="flex-1 min-w-0 lg:pl-8 lg:pr-6">
                <div v-if="currentDoc" class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <!-- Navigation buttons -->
                    <div class="px-6 md:px-10 py-6 border-t border-gray-100 flex items-center justify-between gap-4">
                        <NuxtLink v-if="prevDoc" :to="`/docs/${prevDoc.slug}`"
                            class="flex items-center gap-3 group p-4 rounded-2xl hover:bg-gray-50 border border-gray-100 transition-all max-w-xs">
                        ...
                        <NuxtLink v-if="nextDoc" :to="`/docs/${nextDoc.slug}`"
                            class="flex items-center gap-3 group p-4 rounded-2xl hover:bg-gray-50 border border-gray-100 transition-all max-w-xs text-right">
                            <div class="min-w-0">
            ...
            <aside
                class="hidden xl:flex flex-col w-56 shrink-0 sticky top-[6.5rem] h-[calc(100vh-6.5rem)] overflow-y-auto pl-4 scrollbar-styled">
