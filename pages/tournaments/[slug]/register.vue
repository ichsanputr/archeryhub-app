    ...
    <div class="min-h-screen bg-gray-50 font-body text-navy pb-16">
        ...
        <div v-else-if="!data || !data.event" class="min-h-screen flex items-center justify-center px-4">
            <div class="text-center max-w-md">
                ...
                <BaseButton to="/tournaments" variant="navy" size="md">{{ t('events.register.view_other_events', 'Lihat Event Lainnya') }}</BaseButton>
            </div>
        </div>
        ...
        <template v-else>
            ...
            <main v-if="existingRegistration && existingRegistration.payment_status !== 'cancelled'" class="max-w-3xl mx-auto px-4 py-10 sm:py-14">
                <div class="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-10 shadow-sm space-y-6">
                    ...
                    <div class="space-y-3 pt-2">
                        ...
                        <BaseButton :to="`/tournaments/${slug}`" variant="navy" size="lg" class="w-full justify-center font-bold">
            ...
            <main v-else-if="isAlreadyRegistered" class="max-w-2xl mx-auto px-4 py-12 text-center">
                <div class="bg-white rounded-3xl p-10 border border-gray-200 shadow-sm">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 justify-center">
                        <BaseButton to="/dashboard/archer/tournaments" variant="navy" size="lg" class="px-8">Go to Dashboard
                        </BaseButton>
                        <BaseButton :to="`/tournaments/${slug}`" variant="outline" size="lg" class="px-8">Back to Event
                        </BaseButton>
                    </div>
                </div>
            </main>
            ...
            <main v-else-if="registrationStatus === 'not_started'" class="max-w-2xl mx-auto px-4 py-12 text-center">
                <div class="bg-white rounded-3xl p-10 border border-gray-200 shadow-sm">
                    ...
                    <div class="flex justify-center">
                        <BaseButton :to="`/tournaments/${slug}`" variant="navy" size="lg" class="px-8">Kembali ke Event</BaseButton>
                    </div>
                </div>
            </main>
            ...
            <main v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8 items-start">
                    ...
                    <!-- RIGHT COLUMN — Payment + CTA (Sticky on Desktop) -->
                    <div class="lg:col-span-2 space-y-5 lg:sticky lg:top-24 self-start">
                        ...
                        <section class="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden">
                            ...
                            <!-- Segmented Pill Tab Switcher: 2 Tabs (Online vs Manual) -->
                            <div class="p-4 sm:p-5 pb-0">
                                <div class="bg-slate-100/80 p-1 rounded-2xl flex gap-1 border border-slate-200/60">
                                    <button type="button"
                                        ...
                                        class="flex-1 py-2.5 px-3 text-center rounded-xl text-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer">
                                        <Icon icon="ph:lightning-bold" class="text-base" :class="form.payment_type === 'online' ? 'text-primary' : 'text-slate-400'" />
                                        <span class="truncate">{{ t('my_registration.online_payment', 'Pembayaran Online') }}</span>
                                    </button>
                                    <button v-if="orgManualMethods.length > 0 || paymentMethods.length > 0"
                                        ...
                                        <span class="truncate">{{ t('my_registration.manual_transfer', 'Transfer Manual') }}</span>
                                    </button>
                                </div>
                            </div>

                            <div class="p-4 sm:p-5 pt-3">
                                <!-- Tab Content 1: Online Payment (Mayar vs PayPal Selection) -->
                                <div v-if="form.payment_type === 'online'" class="space-y-3">
                                    <!-- Mayar Domestic Option -->
                                    <div @click="form.online_gateway = 'mayar'"
                                        class="p-4 rounded-2xl border-2 cursor-pointer transition-all space-y-2.5"
                                        :class="form.online_gateway === 'mayar'
                                            ? 'border-primary bg-primary/5 shadow-xs ring-2 ring-primary/20'
                                            : 'border-slate-200 bg-white hover:border-slate-300'">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center gap-3">
                                                <div class="size-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center p-1.5 shrink-0 shadow-2xs">
                                                    <img src="/mayar-logo.png" alt="Mayar" class="w-full h-full object-contain" />
                                                </div>
                                                <div>
                                                    <div class="font-black text-navy text-sm">
                                                        {{ t('my_registration.mayar_gateway_name', 'Mayar (Domestik Indonesia)') }}
                                                    </div>
                                                    <div class="text-[11px] text-slate-500 font-medium">
                                                        {{ t('organizer_subscription.currency_idr', 'Mata Uang: IDR (Rp)') }}
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="size-5 rounded-full flex items-center justify-center text-xs"
                                                :class="form.online_gateway === 'mayar' ? 'bg-primary text-navy font-black' : 'border border-slate-300 text-transparent'">
                                    ...
                                    <div @click="form.online_gateway = 'paypal'"
                                        class="p-4 rounded-2xl border-2 cursor-pointer transition-all space-y-2.5"
                                        :class="form.online_gateway === 'paypal'
                                            ? 'border-blue-500 bg-blue-50/50 shadow-xs ring-2 ring-blue-500/20'
                                            : 'border-slate-200 bg-white hover:border-slate-300'">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center gap-3">
                                                <div class="size-10 rounded-xl bg-white border border-blue-200 flex items-center justify-center p-1.5 shrink-0 shadow-2xs">
                                                    <Icon icon="logos:paypal" class="text-2xl" />
                                                </div>
                                                <div>
                                                    <div class="font-black text-navy text-sm">
                                                        {{ t('my_registration.paypal_gateway_name', 'PayPal (Internasional)') }}
                                                    </div>
                                                    <div class="text-[11px] text-blue-600 font-bold">
                                                        {{ t('organizer_subscription.currency_usd', 'Mata Uang: USD ($)') }}
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="flex items-center gap-2">
                                                <span class="text-xs font-mono font-black text-blue-700 bg-blue-100/80 px-2 py-0.5 rounded-lg border border-blue-200">
                                                    ~${{ (Math.ceil((totalFee / 16000) * 100) / 100).toFixed(2) }} USD
                                                </span>
                                                <div class="size-5 rounded-full flex items-center justify-center text-xs"
                                                    :class="form.online_gateway === 'paypal' ? 'bg-blue-600 text-white font-black' : 'border border-slate-300 text-transparent'">
                                                    <Icon icon="ph:check-bold" />
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="pt-1 flex flex-wrap items-center gap-1.5 text-[10px] font-bold text-slate-700">
                                            <span class="px-2.5 py-1 rounded-lg bg-white border border-blue-200 shadow-2xs">
...
const buttonText = computed(() => {
    if (form.value.payment_type === 'online' && (form.value.payment_amount || 0) > 0) {
        return form.value.online_gateway === 'paypal'
            ? t('my_registration.btn_pay_now_paypal', 'Bayar Sekarang via PayPal')
            : t('my_registration.btn_pay_now_mayar', 'Bayar Sekarang via Mayar')
    }
...
const handleSubmit = async () => {
    ...
    try {
        ...
        const response = await post(`/tournaments/${event.value.id}/participants`, payload)

        // create transaction
        const registrationId = response.registration_id || response.uuid
        const selectedChannel = form.value.payment_type === 'online' ? form.value.online_gateway : 'manual'
