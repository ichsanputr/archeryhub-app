  ...
  <div class="min-h-screen flex flex-col bg-[#f8fafc] text-slate-800 antialiased font-sans">
    ...
    <main class="flex-grow pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full space-y-6">
      ...
      <div class="flex items-center gap-2 text-xs font-bold text-slate-400">
        <NuxtLink to="/tournaments" class="hover:text-navy">Events</NuxtLink>
        <Icon icon="ph:caret-right-bold" />
        <NuxtLink :to="`/tournaments/${slug}`" class="hover:text-navy">{{ event?.name || 'Event' }}</NuxtLink>
        <Icon icon="ph:caret-right-bold" />
...
async function fetchData() {
  ...
  try {
    ...
    const tRes = await get(`/tournaments/${slug.value}/targets`)
