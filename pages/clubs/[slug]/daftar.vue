...
const { data: publicFormData } = await useAsyncData(`club-public-form-${slug}`, () =>
  $fetch(`${config.public.apiBaseUrl}/clubs/${slug}/registration-form`)
)
...
const normalizeFieldType = (fieldType) => {
  ...
  if (fieldType === 'datetime') return 'text'
...
const registrationFields = computed(() => {
  const publishedForm = publicFormData.value?.data
  if (publishedForm?.sections?.length) {
    return publishedForm.sections.flatMap(section => {
      const sectionFields = Array.isArray(section.fields) ? section.fields : []
      return sectionFields
        .filter(field => !['heading', 'paragraph', 'divider'].includes(field.field_type))
        .map(field => ({
          name: field.map_to_field || `field_${field.uuid}`,
          label: field.label || 'Field',
          type: normalizeFieldType(field.field_type),
          placeholder: field.placeholder || '',
          required: Boolean(field.is_required),
          help: field.helper_text || '',
  ...
  // Legacy fallback from clubs.registration_config
