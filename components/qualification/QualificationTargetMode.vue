<template>
    <div class="space-y-6">
        ...
        <!-- Target Auto-Locked Banner when scores exist in category -->
        <div v-if="hasScoresInCategory"
            ...
            <div class="flex-1 min-w-0">
                <div class="text-xs font-black text-slate-900">
                    {{ t('event_qualification.auto_locked_title', 'Penempatan Target Terkunci Otomatis') }}
                </div>
                <div class="text-[11px] text-slate-600 font-medium leading-snug mt-0.5">
                    {{ t('event_qualification.auto_locked_desc', 'Penilaian kualifikasi pada kategori ini telah berlangsung. Pengundian otomatis, reset, dan pemindahan posisi target dinonaktifkan demi menjaga integritas data nilai.') }}
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <BaseButton variant="white" icon="ph:trash-bold" size="md"
                    :disabled="isReseting || isAssigning || hasScoresInCategory || props.archers.length === unassignedArchersCount"
                    :loading="isReseting" class="!text-red-600 !border-red-200 hover:!bg-red-50 font-bold text-sm h-11 px-5"
                    :title="hasScoresInCategory ? t('event_qualification.locked_due_to_scores', 'Target terkunci karena penilaian sudah dimulai') : ''"
                    @click="isSubscriptionActive ? resetAssignments() : (showPremiumModal = true)">
                    {{ t('event_qualification.reset', 'Atur Ulang') }}
                </BaseButton>
 
                <div class="relative dropdown-container" v-click-outside="() => showAutoAssignMenu = false">
                    <BaseButton variant="primary" icon="fa7-solid:random" size="md"
                        :disabled="isAssigning || isReseting || hasScoresInCategory || props.archers.length === 0" :loading="isAssigning"
                        class="font-black text-sm h-11 px-5 shadow-md shadow-primary/20"
                        :title="hasScoresInCategory ? t('event_qualification.locked_due_to_scores', 'Target terkunci karena penilaian sudah dimulai') : ''"
                        @click="isSubscriptionActive && !hasScoresInCategory ? (showAutoAssignMenu = !showAutoAssignMenu) : (hasScoresInCategory ? null : (showPremiumModal = true))">
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="lg:w-80 shrink-0 lg:self-start lg:sticky lg:top-4">
                <div class="bg-slate-100/90 rounded-2xl border border-slate-200/90 p-4 min-h-[520px] flex flex-col gap-3.5 shadow-xs"
                     @dragover.prevent @drop="isSubscriptionActive && !hasScoresInCategory ? handleDropOnUnassigned() : null">
                    ...
                    <div v-else class="flex flex-col gap-0 max-h-[700px] overflow-y-auto custom-scrollbar pr-0.5 pb-6">
                        <div v-for="group in unassignedArchersByClub" :key="group.clubName" class="space-y-2">
                            ...
                            <div v-for="archer in group.archers" :key="archer.uuid" :draggable="isSubscriptionActive && !hasScoresInCategory"
                                @dragstart="(e) => !hasScoresInCategory && handleDragStart(e, archer)" @dragend="handleDragEnd"
                                ...
                                :class="hasScoresInCategory ? 'cursor-not-allowed opacity-80' : 'cursor-grab active:cursor-grabbing hover:border-primary hover:shadow-md'">
                                <div
                                ...
                                <Icon v-if="!hasScoresInCategory" icon="ph:dots-six-vertical-bold"
            ...
            <div class="flex-1">
                ...
                <!-- Target Grid Cards -->
                <div v-else class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 gap-5">
                    <div v-for="target in targetGrid" :key="target.name"
                        class="bg-white rounded-3xl shadow-sm border relative border-slate-200/90 flex flex-col hover:shadow-lg transition-all group/card overflow-visible"
                        :class="{ 'z-[100]': openDropdown?.targetId === target.name }">
                        <!-- Card Header -->
                        <div class="px-5 py-3.5 border-b border-slate-100 bg-gradient-to-r from-slate-50 via-slate-50/80 to-white flex items-center justify-between gap-2 rounded-t-3xl">
                            ...
                            <div class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100/90 hover:bg-slate-200/90 border border-slate-200 text-slate-800 font-mono text-xs font-black shadow-2xs transition-colors"
                                :title="t('event_qualification.board_code_tooltip', 'Kode QR Scan Mobile Scorekeeper')">
                                <Icon icon="ph:qr-code-bold" class="text-sm text-slate-600" />
                                <span>{{ getBoardCode(target.name) ? getBoardCode(target.name) : String(target.name.match(/\d+/)?.[0] || '').padStart(3, '0') + 'TGT' }}</span>
                            </div>
                        </div>
                        ...
                        <div class="p-4 flex-1">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                                <div v-for="pos in target.availableLetters" :key="pos" class="group" @dragover.prevent
                                    @drop="isSubscriptionActive && !hasScoresInCategory ? handleDropOnTarget(target, pos) : null">
                                    
                                    <!-- Filled Slot (Current Category Archer) with Merged Avatar & Absolute Badge -->
                                    <div v-if="target.slots[pos]" :draggable="isSubscriptionActive && !hasScoresInCategory"
                                        @dragstart="(e) => !hasScoresInCategory && handleDragStart(e, target.slots[pos], target, pos)"
                                        @dragend="handleDragEnd"
                                        class="flex items-center gap-2.5 p-2.5 rounded-2xl bg-white border border-slate-200 hover:border-primary/60 transition-all group/slot-filled select-none shadow-2xs"
                                        :class="[hasScoresInCategory ? 'cursor-not-allowed opacity-95' : 'cursor-grab active:cursor-grabbing hover:shadow-md', { 'opacity-50': draggedArcher?.archer.uuid === target.slots[pos].uuid }]">
                                        
                                        <!-- Merged Avatar with Position Badge -->
                                        <div class="relative size-10 rounded-xl overflow-visible shrink-0 pointer-events-none">
                                            <div class="size-10 rounded-xl border border-slate-200 overflow-hidden bg-slate-50 shadow-2xs">
                                            ...
                                            <span class="absolute -bottom-1 -left-1 size-5 rounded-md bg-navy text-primary text-[10px] font-black flex items-center justify-center shadow-xs border border-white">
                                                {{ pos }}
                                            </span>
                                        </div>
                                        ...
                                        <div class="flex-1 min-w-0 pointer-events-none">
                                            <div class="flex items-center gap-1 justify-between">
                                                <div class="text-xs font-black text-slate-900 truncate leading-snug">{{ target.slots[pos].name }}</div>
                                            </div>
                                            <div class="flex items-center gap-1.5 mt-0.5">
                                                <div class="text-[10px] text-slate-500 truncate font-semibold">
                                                    {{ target.slots[pos].club || t('event_qualification.independent', 'Independen') }}
                                                </div>
                                            </div>
                                        </div>

                                        <!-- Remove Archer Button -->
                                        <button v-if="!hasScoresInCategory" type="button"
                                    ...
                                    <!-- Empty Dropzone Slot with Merged Badge Icon -->
                                    <div v-else class="relative archer-dropdown-container"
                                        v-click-outside="() => { if (openDropdown?.targetId === target.name && openDropdown?.pos === pos) openDropdown = null }"
                                        @dragover.prevent
                                        @drop.stop="isSubscriptionActive && !hasScoresInCategory ? handleDropOnTarget(target, pos) : null">
                                        <div @click.stop="isSubscriptionActive && !hasScoresInCategory ? toggleDropdown(target.name, pos) : (hasScoresInCategory ? null : (showPremiumModal = true))"
                                            class="flex items-center gap-2.5 p-2.5 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/50 transition-all group/slot min-h-[58px]"
                                            :class="[hasScoresInCategory ? 'cursor-not-allowed opacity-50 bg-slate-100' : 'cursor-pointer hover:bg-white hover:border-primary hover:shadow-xs', { 'border-primary bg-primary/5 ring-4 ring-primary/10 shadow-inner': isDragging && !hasScoresInCategory }]">
                                            
                                            <!-- Merged Placeholder Box with Badge -->
                                            <div class="relative size-10 rounded-xl overflow-visible shrink-0 pointer-events-none">
                                                <div class="size-10 rounded-xl border-2 border-dashed border-slate-300 bg-white flex items-center justify-center text-slate-300 group-hover/slot:border-primary group-hover/slot:text-primary transition-colors">
                                                    <Icon icon="ph:user-plus-bold" class="text-base" />
                                                </div>
                                                <span class="absolute -bottom-1 -left-1 size-5 rounded-md bg-slate-200 text-slate-600 text-[10px] font-black flex items-center justify-center shadow-xs border border-white group-hover/slot:bg-navy group-hover/slot:text-primary transition-colors">
                                                    {{ pos }}
                                                </span>
                                            </div>

                                            <div class="flex-1 min-w-0">
                                                <div class="text-xs font-bold text-slate-400 group-hover/slot:text-navy transition-colors">
                                                    {{ hasScoresInCategory ? t('event_qualification.locked', 'Terkunci') : t('event_qualification.select_or_drop', '+ Isi Pemanah') }}
                                                </div>
                                                <div v-if="!hasScoresInCategory" class="text-[9px] text-slate-400 font-medium">
                                                    {{ t('event_qualification.drop_hint', 'Klik atau tarik ke sini') }}
                                                </div>
                                            </div>
                                            <Icon v-if="!hasScoresInCategory" icon="ph:plus-circle-bold"
                                                class="text-slate-300 text-lg group-hover/slot:text-primary transition-colors shrink-0" />
                                            <Icon v-else icon="ph:lock-key-bold"
                                                class="text-slate-300 text-sm shrink-0" />
                                        </div>

                                        <!-- Dropdown Archer Picker -->
                                        <div v-if="openDropdown?.targetId === target.name && openDropdown?.pos === pos"
                                            class="absolute !z-[10000] mt-1.5 w-full min-w-[280px] bg-white rounded-2xl shadow-2xl border border-slate-200 py-2 left-0 top-full">
                                            <div class="px-3 pb-2 border-b border-slate-100">
                                                <div class="relative">
                                                    ...
                                                    <input v-model="filterText" type="text" :placeholder="t('event_qualification.search_archer', 'Cari pemanah...')"
                                                        class="w-full pl-8 pr-3 py-1.5 text-xs border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
                                                        @click.stop />
                                                </div>
                                            </div>
                                            <div class="max-h-60 overflow-y-auto pt-1 custom-scrollbar">
                                                <div v-if="unassignedArcherListFiltered.length === 0"
                                                    class="px-4 py-3 text-center text-slate-400 text-xs font-bold">
                                                ...
                                                <BaseButton v-for="archer in unassignedArcherListFiltered"
                                                    ...
                                                    class="w-full px-3.5 py-2 text-left hover:bg-slate-50 flex items-center gap-2.5 transition-colors border-none border-b border-slate-50 last:border-0 group/archer-item shadow-none justify-start">
                                                    <div class="size-8 rounded-xl border border-slate-100 overflow-hidden shrink-0 bg-slate-50">
                                                        <img :src="useImageOrDefault(archer.avatar_url, archer.name)"
                                                            class="w-full h-full object-cover" />
                                                    </div>
                                                    <div class="flex-1 min-w-0">
                                                        <div class="text-xs font-black text-slate-900 truncate leading-tight mb-0.5">
                                                            {{ archer.name }}
                                                        </div>
                                                        <div class="text-[10px] text-slate-500 font-semibold truncate tracking-tight">
                                                            {{ archer.club || t('event_qualification.independent', 'Independen') }}
                                                        </div>
                                                    </div>
                                                </BaseButton>
    ...
    <Teleport to="body">
      <Transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0"
        ...
        <div v-if="showAutoAssignDialog" class="fixed inset-0 z-[10000] flex items-center justify-center p-4"
          @click.self="showAutoAssignDialog = false">
          <!-- Backdrop -->
    ...
    <Teleport to="body">
      <Transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0"
        enter-to-class="opacity-100" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100"
        leave-to-class="opacity-0">
        <div v-if="showResetDialog" class="fixed inset-0 z-[10000] flex items-center justify-center p-4"
    ...
    <Teleport to="body">
      <Transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0"
        enter-to-class="opacity-100" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100"
        leave-to-class="opacity-0">
        <div v-if="showMoveScoredModal" class="fixed inset-0 z-[10000] flex items-center justify-center p-4"
          @click.self="showMoveScoredModal = false">
    ...
    <Teleport to="body">
      <Transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0"
        enter-to-class="opacity-100" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100"
        leave-to-class="opacity-0">
        <div v-if="showUnassignScoredModal" class="fixed inset-0 z-[10000] flex items-center justify-center p-4"
          @click.self="showUnassignScoredModal = false">
...
const hasScoresInCategory = computed(() => {
