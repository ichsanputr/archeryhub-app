...
gap-6">
            <div v-for="(file, index) in results" :key="index" class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-lg hover:border-primary/30 transition-all
group">
                <div class="flex items-start gap-4">
                    <div
                        class="w-14 h-14 rounded-xl bg-navy/5 flex items-center justify-center flex-shrink-0 group-hover:bg-navy group-hover:text-white transition-all">
                        <Icon :icon="getFileIcon(file.url)" class="text-2xl text-navy group-hover:text-white" />
                    </div>
                    <div class="flex-1 min-w-0">
                        <h3 class="font-bold text-navy text-sm mb-1 truncate"
                            :title="file.name || getFileName(file.url)">
                            {{ file.name || getFileName(file.url) }}
                            \u003cdiv v-else class=\"bg-white rounded-2xl p-12 shadow-sm border border-gray-100
                            text-center\"\u003e
                            \u003cdiv class=\"w-24 h-24 mx-auto rounded-full bg-gray-100 flex items-center
                            justify-center mb-6\"\u003e
