    ...
    <svg class="fixed inset-0 w-full h-full pointer-events-none z-[998]">
      <defs>
        <mask id="tour-spotlight-mask">
          ...
          <rect :x="spotlightRect.left - padding" :y="spotlightRect.top - padding"
            :width="spotlightRect.width + padding * 2" :height="spotlightRect.height + padding * 2" :rx="radius"
            fill="black" class="transition-all duration-300 ease-out" />
        </mask>
      </defs>
      <!-- Semi-transparent overlay blocking background interaction -->
      <rect x="0" y="0" width="100%" height="100%" fill="rgba(15, 23, 42, 0.75)" mask="url(#tour-spotlight-mask)"
        class="pointer-events-auto" />
    </svg>

    <!-- Interactive spotlight helper overlay to capture focus of element -->
    <div class="fixed border-2 border-primary pointer-events-none transition-all duration-300 ease-out z-[999]" :style="{
      ...
      <!-- Pulsing border ring -->
      <div class="absolute inset-0 border border-primary animate-ping opacity-75"
        :style="{ borderRadius: `${radius}px` }"></div>
    </div>
    ...
    <div ref="cardRef"
      ...
      <div class="flex flex-col gap-1">
        <div class="flex items-center justify-between mb-1">
          <span class="text-[9px] font-black text-primary tracking-[0.2em]">
      ...
      <div class="flex items-center justify-between pt-3 border-t border-white/10">
        <button v-if="!isFirstStep" @click="prevStep"
          class="flex items-center gap-1.5 text-[10px] font-black text-slate-400 hover:text-white transition-all tracking-wider">
        ...
        <button @click="nextStep"
          class="px-4 py-2 bg-primary hover:bg-white text-navy font-black text-[10px] rounded-xl transition-all tracking-widest flex items-center gap-1.5 shadow-md shadow-primary/20 active:scale-95">
