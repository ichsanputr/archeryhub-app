    ...
    <Transition name="chat-window">
      ...
      <header class="bg-navy px-5 py-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="size-10 bg-white/15 rounded-full flex items-center justify-center text-white border border-white/20">
          ...
          <div>
            ...
            <div class="flex items-center gap-1.5 mt-1">
              ...
              <span class="text-white/60 text-[11px] font-medium uppercase tracking-wider">Sedang Online</span>
            </div>
          </div>
        </div>
      ...
      <div ref="chatContainer" class="flex-1 overflow-y-auto p-5 space-y-4 bg-slate-50/50">
        <TransitionGroup name="chat-message" tag="div" class="space-y-4">
          <div v-for="(message, index) in messages" :key="`msg-${index}-${message.time}`" :class="message.role === 'bot' ? 'flex items-end gap-2.5' : 'flex items-end justify-end gap-2.5'">
          <template v-if="message.role === 'bot'">
            ...
            <div class="flex flex-col gap-1 max-w-[80%]">
              <div class="bg-navy text-white px-4 py-3 rounded-2xl rounded-bl-none text-sm leading-relaxed shadow-sm">{{ message.text }}</div>
          ...
          <template v-else>
            <div class="flex flex-col gap-1 items-end max-w-[80%]">
              <div class="bg-gray-100 text-navy px-4 py-3 rounded-2xl rounded-br-none text-sm leading-relaxed shadow-sm border border-slate-200">{{ message.text }}</div>
              <span class="text-[10px] text-slate-400 mr-1">{{ message.time }}</span>
            </div>
            <div class="size-8 rounded-full bg-primary/20 flex items-center justify-center text-navy shrink-0 border border-primary/30">
...
const sendMessage = async (prefilled?: string) => {
  ...
  try {
    ...
    const res = await $fetch<{ answer?: string; quick_actions?: string[] }>(`${config.public.apiBaseUrl}/chatbot/message`, {
      method: 'POST',
      body: { message: text },
    })
