            ...
            <div class="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1 divide-y divide-slate-100" style="max-height: calc(90vh - 140px);">
              ...
              <div class="space-y-3">
                <label class="block text-xs sm:text-sm font-bold text-navy">
                  {{ t('archer_payments_list.filter_status_label') }}
                </label>
              ...
              <div class="pt-5 space-y-3">
                <label class="block text-xs sm:text-sm font-bold text-navy">
                  {{ t('archer_payments_list.filter_method_label') }}
                </label>
                <div class="grid grid-cols-2 gap-2.5">
                  <button
                    ...
                    :key="opt.value"
                    type="button"
                    @click="draftFilters.method = opt.value"
                    :class="[
                      'py-2.5 px-3 rounded-xl border text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-2 cursor-pointer',
                      draftFilters.method === opt.value
                        ? 'bg-white border-navy text-navy font-black shadow-2xs ring-1 ring-navy/20'
                        : 'bg-slate-50 text-slate-700 border-slate-200/90 hover:bg-slate-100 hover:border-slate-300'
                    ]"
                  >
                    <Icon v-if="draftFilters.method === opt.value" icon="ph:check-bold" class="text-xs text-navy shrink-0" />
                    <span class="truncate">{{ opt.label }}</span>
                  </button>
                </div>
              </div>
              ...
              <div class="pt-5 space-y-3">
                <label class="block text-xs sm:text-sm font-bold text-navy">
                  {{ t('archer_payments_list.filter_sort_label') }}
                </label>
                <div class="grid grid-cols-2 gap-2.5">
                  <button
...
const statusOptions = computed(() => [
  ...
  { label: t('archer_payments_list.opt_paid'), value: 'paid' },
  { label: t('archer_payments_list.opt_awaiting'), value: 'awaiting_verification' },
  { label: t('archer_payments_list.opt_pending'), value: 'pending' },
  { label: t('archer_payments_list.opt_failed'), value: 'failed' }
])
...
const sortOptions = computed(() => [
  { label: t('archer_payments_list.sort_newest'), value: 'date_desc' },
  { label: t('archer_payments_list.sort_oldest'), value: 'date_asc' },
  { label: t('archer_payments_list.sort_highest'), value: 'amount_desc' },
  { label: t('archer_payments_list.sort_lowest'), value: 'amount_asc' }
])
  ...
  (newVal) => {
    if (newVal) {
      draftFilters.value = {
        ...
        method: props.currentFilters.method || 'all',
...
const draftActiveFilterCount = computed(() => {
  ...
  if (draftFilters.value.method && draftFilters.value.method !== 'all') count++
