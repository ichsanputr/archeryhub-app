            ...
            <div class="relative p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="space-y-3">
                    <div class="flex items-center gap-2 text-[10px] sm:text-xs font-black tracking-widest text-primary/80">
                        ...
                        <NuxtLink to="/dashboard/root/articles" class="text-white hover:text-primary transition-colors">{{ t('root.articles.title', 'Articles') }}</NuxtLink>
                        <Icon icon="ph:caret-right-bold" class="text-[8px] opacity-50" />
                        <span class="text-primary">{{ isEdit ? t('root.articles.edit_title', 'Edit Article') : t('root.articles.create_title', 'Create Article') }}</span>
                    </div>
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-xl sm:text-3xl font-black tracking-tight">
                                {{ isEdit ? (form.title || t('root.articles.edit_title', 'Edit Article')) : t('root.articles.create_title', 'Create New Article') }}
                            </h1>
                            <div class="text-slate-300 text-[10px] sm:text-sm font-medium mt-0.5">
                                {{ isEdit ? t('root.articles.edit_subtitle', 'Update article information and publication settings') : t('root.articles.create_subtitle', 'Write and publish educational guides or news') }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <BaseButton v-if="form.status !== 'published'" variant="outline" @click="handleSave('draft')" :loading="savingDraft" icon="ph:floppy-disk-bold" class="!text-white !border-white/20 hover:!bg-white/10">
                        {{ t('root.articles.save_draft', 'Simpan Draf') }}
                    </BaseButton>
                    <BaseButton variant="primary" @click="handleSave(form.status === 'draft' ? 'published' : form.status)" :loading="submitting" icon="ph:paper-plane-tilt-bold" class="shadow-lg shadow-primary/20">
                        {{ isEdit ? t('root.articles.update', 'Perbarui Artikel') : t('root.articles.publish', 'Publikasikan') }}
                    </BaseButton>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-xs space-y-5">
                    <div>
                        <label class="block text-xs font-bold text-navy mb-2 tracking-wider">
                            {{ t('root.articles.form_title', 'Judul Artikel') }} <span class="text-red-500">*</span>
                        </label>
                        <input v-model="form.title" @input="onTitleInput" type="text"
                            :placeholder="t('root.articles.form_title_placeholder', 'Contoh: Panduan Memilih Busur Recurve untuk Pemula')"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="flex items-center rounded-xl border border-gray-200 bg-gray-50/70 overflow-hidden focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
                            ...
                            <input v-model="form.slug" :readonly="!isManualSlug" type="text"
                                :placeholder="t('root.articles.form_slug_placeholder', 'panduan-memilih-busur-recurve')"
                    ...
                    <div>
                        <label class="block text-xs font-bold text-navy mb-2 tracking-wider">
                            {{ t('root.articles.form_excerpt', 'Ringkasan Singkat (Excerpt)') }}
                        </label>
                        <textarea v-model="form.excerpt" rows="2"
                            :placeholder="t('root.articles.form_excerpt_placeholder', 'Tuliskan ringkasan 1-2 kalimat mengenai isi artikel...')"
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-xs space-y-4">
                    <div class="flex items-center justify-between border-b border-gray-100 pb-3">
                        <label class="block text-xs font-black text-navy tracking-wider">
                            {{ t('root.articles.form_content', 'Konten Artikel') }} <span class="text-red-500">*</span>
                        </label>
                        ...
                        <div class="flex items-center bg-gray-100 p-1 rounded-xl gap-1">
                            <button type="button" @click="activeTab = 'write'"
                                ...
                                <span>{{ t('root.articles.form_tab_write', 'Tulis') }}</span>
                            </button>
                            <button type="button" @click="activeTab = 'preview'"
                                ...
                                <span>{{ t('root.articles.form_tab_preview', 'Pratinjau') }}</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div v-show="activeTab === 'write'" class="flex flex-wrap items-center gap-1 p-2 bg-gray-50/80 rounded-2xl border border-gray-100 text-gray-600">
                        ...
                        <div class="h-4 w-px bg-gray-200 mx-1"></div>
                        <button type="button" @click="insertFormat('ul')" title="Bullet List"
                        ...
                        <button type="button" @click="insertFormat('ol')" title="Numbered List"
                    ...
                    <div v-show="activeTab === 'write'">
                        <textarea ref="editorRef" v-model="form.content" rows="18"
                            :placeholder="t('root.articles.form_content_placeholder', 'Tulis konten artikel di sini. Mendukung pemformatan Markdown & HTML...')"
                    ...
                    <div v-show="activeTab === 'preview'" class="min-h-[400px] p-6 bg-slate-50/60 rounded-2xl border border-gray-200">
                        <div v-if="!form.content" class="text-center py-16 text-gray-400 text-sm">
                            Belum ada konten untuk ditampilkan di pratinjau.
                        </div>
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-xs space-y-3">
                    <label class="block text-xs font-bold text-navy tracking-wider">
                        {{ t('root.articles.form_tags', 'Tag / Kata Kunci') }}
                    </label>
                    <div class="flex flex-wrap items-center gap-2 p-2.5 border border-gray-200 rounded-2xl min-h-[48px] bg-white focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
                        ...
                        <input v-model="tagInput" @keydown.enter.prevent="addTag" @keydown.comma.prevent="addTag"
                            type="text" :placeholder="t('root.articles.form_tags_placeholder', 'Ketik tag lalu tekan Enter')"
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-xs space-y-4">
                    ...
                    <BaseSelect v-model="form.status" :options="statusOptions" :label="t('root.articles.form_status', 'Status Publikasi')" class="w-full" />

                    <div>
                        <label class="block text-xs font-bold text-gray-500 mb-1.5 tracking-wider">
                            {{ t('root.articles.form_published_at', 'Waktu Publikasi') }}
                        </label>
                    ...
                    <div class="pt-2 border-t border-gray-100 flex flex-col gap-2">
                        ...
                        <BaseButton v-if="form.status !== 'draft'" variant="outline" block @click="handleSave('draft')" :loading="savingDraft" icon="ph:floppy-disk-bold">
                            {{ t('root.articles.save_draft', 'Simpan sebagai Draf') }}
                        </BaseButton>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-xs space-y-4">
                    ...
                    <BaseSelect v-model="form.category" :options="categoryListOptions" :label="t('root.articles.form_category', 'Kategori')" class="w-full" />

                    <!-- Custom Category if needed -->
                    <div v-if="form.category === 'custom'" class="pt-2">
                        <label class="block text-xs font-bold text-gray-500 mb-1.5 tracking-wider">Kategori Kustom Baru</label>
                        <input v-model="customCategory" type="text" placeholder="Masukkan nama kategori baru..."
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-xs space-y-4">
                    ...
                    <div class="aspect-[16/9] rounded-2xl overflow-hidden bg-gray-100 border border-gray-200 flex items-center justify-center relative group">
                        ...
                        <div v-else class="flex flex-col items-center gap-2 text-gray-300">
                            ...
                            <span class="text-[10px] font-bold">16:9 Banner Image</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center gap-2">
                            ...
                            <BaseButton type="button" variant="outline" size="sm" block @click="fileInputRef?.click()" :loading="uploadingImage" icon="ph:upload-simple-bold">
                                {{ uploadingImage ? t('root.articles.form_uploading', 'Mengunggah...') : t('root.articles.form_upload_cover', 'Unggah Cover') }}
                            </BaseButton>
                        </div>
                        <input v-model="form.image_url" type="text" placeholder="Atau paste URL cover image..."
                        ...
                        <div class="text-[10px] text-gray-400 leading-tight">
                            {{ t('root.articles.form_cover_image_desc', 'Rekomendasi rasio 16:9 (1200x675 px). Maksimal 5MB.') }}
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-xs space-y-4">
                    <h3 class="text-xs font-black text-navy uppercase tracking-wider flex items-center gap-2">
                        ...
                        <span>Metadata Penulis</span>
                    </h3>

                    <div>
                        <label class="block text-xs font-bold text-gray-500 mb-1.5 tracking-wider">
                            {{ t('root.articles.form_author_name', 'Nama Penulis') }}
                        </label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500 mb-1.5 tracking-wider">
                            {{ t('root.articles.form_author_role', 'Peran Penulis') }}
                        </label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500 mb-1.5 tracking-wider">
                            {{ t('root.articles.form_read_time', 'Estimasi Waktu Baca (Menit)') }}
                        </label>
...
const form = ref({
    ...
    author_avatar: '/profile-author.png',
    tags: [],
    read_time: 5,
    status: 'published',
    published_at: ''
...
const handleFileUpload = async (event) => {
    ...
    try {
        ...
        toast.success(t('root.articles.form_cover_image', 'Cover banner berhasil diunggah'))
    } catch (err) {
...
const fetchArticleDetail = async () => {
    ...
    try {
        ...
        form.value = {
            ...
            author_avatar: data.author_avatar || '/profile-author.png',
...
const handleSave = async (targetStatus) => {
    if (!form.value.title.trim()) {
        toast.error(t('root.articles.form_title', 'Judul artikel wajib diisi'))
        return
    }
    if (!form.value.content.trim()) {
        toast.error(t('root.articles.form_content', 'Konten artikel tidak boleh kosong'))
        return
    }
    ...
    try {
        if (isEdit.value) {
            ...
            toast.success(t('root.articles.update_success', 'Artikel berhasil diperbarui'))
        } else {
            await api.post('/root/dashboard/articles', payload)
            toast.success(t('root.articles.save_success', 'Artikel berhasil dibuat'))
        }
        router.push('/dashboard/root/articles')
    } catch (err) {
