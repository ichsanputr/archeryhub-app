<template>
  <div class="space-y-6">
    <!-- Header Section -->
    <DashboardHeader
      :title="t('my_events.title', 'Event & Turnamen Saya')"
      ...
      :breadcrumbs="[
        ...
        { label: t('my_events.title', 'Event Saya') }
      ]"
    >
      <template #actions>
        <BaseButton to="/events" variant="primary" icon="ph:magnifying-glass-bold"
          class="h-11 px-5 font-black text-xs shadow-sm hover:shadow-md transition-all">
          {{ t('my_events.find_new_events', 'Cari Event Baru') }}
        </BaseButton>
      </template>
    </DashboardHeader>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
      <!-- Total Events -->
      <StatCard
        :title="t('my_events.stats_total', 'Total Event')"
        :value="events.length"
        icon="ph:trophy-bold"
        color="primary"
        ...
        <div v-for="event in filteredEvents" :key="event.id"
            ...
            <div class="relative z-10 flex items-start gap-3.5">
              ...
              <div class="min-w-0 flex-1 space-y-1">
                <div class="flex items-center justify-between gap-2">
                  <!-- Event Title -->
                  <NuxtLink :to="`/dashboard/archer/events/${event.slug || event.id}/overview`" class="block flex-1 min-w-0">
          ...
          <div class="p-6 space-y-6 flex-1 flex flex-col justify-between">
            ...
            <div class="pt-3 flex items-center gap-2.5 border-t border-slate-100">
              <NuxtLink :to="`/dashboard/archer/events/${event.slug || event.id}/overview`" class="flex-1">
          ...
          <Transition
            ...
            <div v-if="showQR" class="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl border border-slate-100 p-6 flex flex-col items-center text-center space-y-4 my-auto" @click.stop>
              ...
              <div class="w-full space-y-2 pt-1">
                <NuxtLink :to="`/dashboard/archer/events/${selectedEvent?.slug || selectedEvent?.id}/my-registration`" class="block w-full">
