<template>
    <section class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm relative overflow-hidden group transition-all"
            ...
            <h3 class=" text-xs font-black text-gray-500 tracking-widest mb-6">{{ t('subscription.status.current_plan_title', 'Paket Saat Ini') }}</h3>
            <div class="mb-6">
                ...
                <div class="text-sm text-gray-500">{{ t('subscription.status.billed', 'Ditagih') }} {{ billingCycle }}</div>
            </div>
            <div v-if="!isExpired" class="space-y-4 mb-6">
                <div class="flex justify-between items-center text-sm">
                    <span class="text-gray-500">{{ t('subscription.status.price', 'Harga') }}</span>
                    <span class="font-bold text-navy">{{ priceLabel }}</span>
                </div>
                <div class="flex justify-between items-center text-sm">
                    <span class="text-gray-500">{{ t('subscription.status.next_billing', 'Tagihan Berikutnya') }}</span>
        ...
        <div class="xl:col-span-2 bg-white p-6 rounded-2xl border shadow-sm transition-all"
            ...
            <h3 class=" text-xs font-black text-gray-500 tracking-widest mb-6">{{ t('subscription.usage.title', 'Pemakaian & Batas') }}</h3>
            <div v-if="isExpired" class="flex flex-col items-center justify-center py-8 text-center gap-3">
                ...
                <div class="text-sm font-black text-navy tracking-widest">{{ t('subscription.usage.locked', 'Data Terkunci') }}</div>
                <div class="text-xs text-gray-400 font-medium leading-relaxed">{{ t('subscription.usage.unlock_message', 'Perbarui subscription Anda untuk membuka kembali statistik pemakaian.') }}</div>
            </div>
            <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-end">
                        <div>
                            <div class="text-xs font-black text-gray-400 tracking-widest">{{ t('subscription.usage.media_storage', 'Penyimpanan Media') }}
                            </div>
                ...
                <div v-if="showMembers" class="space-y-3">
                    <div class="flex justify-between items-end">
                        <div>
                            <div class="text-xs font-black text-gray-400 tracking-widest">{{ memberLabel || t('subscription.usage.total_members', 'Total Anggota') }}</div>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-end">
                        <div>
                            <div class="text-xs font-black text-gray-400 tracking-widest">{{ t('subscription.usage.active_period', 'Masa Aktif') }}</div>
