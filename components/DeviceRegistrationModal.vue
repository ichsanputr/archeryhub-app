<template>
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background-dark/80 backdrop-blur-sm">
    <div class="card max-w-lg w-full shadow-2xl border-primary/20 relative overflow-hidden">
      ...
      <div class="relative">
        ...
        <div v-if="!registeredDevice" class="space-y-4">
          <div>
            <label class="block text-brand-gold text-xs font-bold uppercase mb-1.5 pl-1">Target Tournament</label>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-brand-gold text-xs font-bold uppercase mb-1.5 pl-1">Device Name</label>
              <input v-model="form.device_name" type="text" class="input w-full" placeholder="e.g. Tablet 01" />
            </div>
            <div>
              <label class="block text-brand-gold text-xs font-bold uppercase mb-1.5 pl-1">Device Type</label>
          ...
          <div class="pt-4">
            <button 
              ...
              :disabled="loading || !form.tournament_id"
        ...
        <div v-else class="flex flex-col items-center py-6 text-center">
          ...
          <div class="space-y-2 mb-8">
            ...
            <p class="text-brand-gold text-sm max-w-xs mx-auto">This code will link this device to <span class="text-white">{{ form.device_name || 'this scoring station' }}</span></p>
          </div>

          <div class="grid grid-cols-2 gap-4 w-full px-8">
            <div class="bg-surface-highlight p-3 rounded-lg border border-white/5">
              <p class="text-[10px] uppercase text-brand-gold font-bold mb-1">Device Code</p>
              <p class="text-white font-mono font-bold tracking-widest text-lg">{{ registeredDevice.device_code }}</p>
            </div>
            <div class="bg-surface-highlight p-3 rounded-lg border border-white/5">
              <p class="text-[10px] uppercase text-brand-gold font-bold mb-1">PIN</p>
              <p class="text-primary font-mono font-bold tracking-widest text-lg">{{ registeredDevice.pin }}</p>
            </div>
          </div>

          <button @click="resetAndClose" class="mt-8 text-brand-gold text-sm font-bold hover:text-white transition-colors">
